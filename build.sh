...
# Force loop-safe behavior even if upstream kickstart has plain 'reboot'
for p in inst.kexec inst.reboot=0; do
  if [[ " $KERNEL_PARAMS " != *" $p "* ]]; then
    KERNEL_PARAMS="$KERNEL_PARAMS $p"
  fi
done
